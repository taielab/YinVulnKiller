...
- **多平台支持**：集成 FOFA、Hunter 等主流测绘平台
- **统一接口**：标准化资产数据格式，自动去重聚合
- **智能联动**：测绘结果自动导入扫描流程
- **灵活查询**：支持 IP/域名/语法/文件多种查询方式
...
YinVulKiller 集成了 FOFA、Hunter 等主流网络空间测绘平台，支持自动资产发现与联动扫描：

```bash
# 1. 使用 FOFA 语法查询并扫描
./yinvulkiller scan -c 'title="登录"' --size 500

# 2. 指定测绘平台
./yinvulkiller scan -c 'domain="example.com"' --platform fofa --size 100

# 3. 多平台联合查询（自动去重）
./yinvulkiller scan -c 'app="Apache"' --platform fofa,hunter --size 200

# 4. 查询 IP 或域名（自动识别）
./yinvulkiller scan -c '192.168.1.1' --size 50
./yinvulkiller scan -c 'example.com' --size 50

# 5. 从文件导入查询语句
./yinvulkiller scan -c queries.txt --platform fofa --size 1000
```
...
| 参数 | 说明 | 示例 |
|------|------|------|
| `-c, --cyber` | 测绘查询（IP/域名/语法/文件） | `-c 'title="admin"'` |
| `--platform` | 指定平台（fofa,hunter） | `--platform fofa` |
| `--size` | 查询数量（默认 100） | `--size 500` |
...
| `-c, --cyber` | 网络空间测绘查询 | `-c 'title="login"'` |
| `--platform` | 测绘平台（fofa,hunter） | `--platform fofa` |
| `--size` | 测绘查询数量 | `--size 500` |
...
```bash
# FOFA 语法查询 + 漏洞扫描
./yinvulkiller scan -c 'title="登录系统"' --platform fofa --size 500 \
  --engine nuclei --nuclei-severity critical,high

# Hunter 平台查询 + 弱口令爆破
./yinvulkiller scan -c 'app="MySQL"' --platform hunter --size 200 --dbs

# 多平台联合查询（自动去重）
./yinvulkiller scan -c 'domain="example.com"' --platform fofa,hunter --size 300
```
...
- 🔥 **新增 Hunter 平台支持** - 鹰图平台集成，多维度资产发现
- ✅ 统一资产数据格式，自动去重聚合
- ✅ 支持多平台联合查询，结果自动合并
- ✅ 灵活查询方式：IP/域名/语法/文件自动识别

**⚡ 扫描引擎优化**：
- ✅ 重构 Nuclei 扫描引擎分布式架构，提升并发效率
- ✅ 完善 GoPOC 扫描插件，增强检测能力
- ✅ 重构指纹识别模块，提升准确率
- ✅ 优化 Spy 模块精准度，减少误报

**🔐 弱口令扫描增强**：
- ✅ 修复 Rsync 服务弱口令扫描未生效问题
- ✅ 修复爆破稳定性问题，优化超时控制
- ✅ 完善账户级去重机制，确保结果准确
